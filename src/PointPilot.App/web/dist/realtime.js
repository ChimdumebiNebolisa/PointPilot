...
async function connect(clientSecret) {
    disconnect(false);
    ...
    nextChannel.addEventListener("message", event => handleRealtimeEvent(JSON.parse(String(event.data))));
    ...
    if (!response.ok)
        throw new Error(`Realtime connection failed with HTTP ${response.status}.`);
...
function sendToolResult(callId, output, createResponse) {
...
window.chrome.webview.addEventListener("message", event => {
    ...
    try {
        switch (message.type) {
            case "connect":
                void connect(message.clientSecret).catch(() => post({ type: "error" }));
                break;
            case "mute":
                setMuted(message.muted);
                break;
            case "tool_result":
                sendToolResult(message.callId, message.output, true);
                break;
            case "tool_interrupted":
                sendToolResult(message.callId, message.output, false);
                break;
            case "cancel_response":
                if (channel?.readyState === "open" && responseActive)
